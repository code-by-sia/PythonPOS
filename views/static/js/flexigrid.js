...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		var g = {
			...
			var cdright = 0 - this.hDiv.scrollright;
			if (this.hDiv.scrollright>0) cdright -= Math.floor(p.cgwidth/2);
			...
			$('thead tr:first th:visible',this.hDiv).each
				(
			 	function ()
					...
					var ppos = cdpos;
					if (cdright==0) 
							cdright -= Math.floor(p.cgwidth/2); 

					cdpos = cdpos + cdright + cdpad;
					
					$('div:eq('+n+')',g.cDrag).css({'right':cdpos+'px'}).show();
				...
				if (dragtype=='colresize') //column resize
					{
						...
						this.colresize = {startX: e.pageX, ol: parseInt(obj.style.right), ow: ow, n : n };
				...
				else if (dragtype=='colMove') //column header drag
					{
						...
						this.hset.left = this.hset.right + $('table',this.hDiv).width();
						...
						$(this.colCopy).css({position:'absolute',float:'right',display:'none', textAlign: obj.align});
				...
				if (this.colresize) //column resize
					{
						...
						var nright = this.colresize.ol + diff;
						var nw = this.colresize.ow + diff;
						if (nw > p.minwidth)
							{
								$('div:eq('+n+')',this.cDrag).css('right',nright);
					...
					if (e.pageX > this.hset.left || e.pageX < this.hset.right || e.pageY > this.hset.bottom || e.pageY < this.hset.top)
					{
						//this.dragEnd();
					...
					$(this.colCopy).css({top:e.pageY + 10,right:e.pageX + 20, display: 'block'});
				}													
				...
				if (this.colresize)
					{
						...
						var nw = this.colresize.nw;
								...
								this.hDiv.scrollright = this.bDiv.scrollright;
			...
			getCellDim: function (obj) // get cell prop for editable event
			{
				...
				var right = obj.offsetParent.offsetright;
				var pdl = parseInt($(obj).css('paddingright'));
				var pdt = parseInt($(obj).css('paddingTop'));
				return {ht:ht,wt:wt,top:top,right:right,pdl:pdl, pdt:pdt, pht:pht, pwt: pwt};
			},
			addRowProp: function()
			{
						...
						.hover(
							function(){
								...
								if (g.colCopy) 
									...
									if (n<g.dcoln) $(this).append(g.cdropright);
									else $(this).append(g.cdropleft);
									...
									var onl = parseInt($('div:eq('+nv+')',g.cDrag).css('right'));
									var nw = parseInt($(g.nBtn).width()) + parseInt($(g.nBtn).css('borderrightWidth'));
